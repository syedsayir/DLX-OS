...
int LockRelease(Lock *k) {
	...
	if (!AQueueEmpty(&k->waiting)) { // there is a process to wake up
		...
		//printf ( "LockRelease by Pid: %d : Waking up PID %d, assigning lock.\n", GetCurrentPid(), (int)(GetPidFromAddress(pcb)));
		k->pid = GetPidFromAddress(pcb);
		ProcessWakeup (pcb);
	}
	//else 
	//printf("NOTHING TO WAKEUP LOCK %d\n",GetCurrentPid());
	RestoreIntrs (intrs);
	return SYNC_SUCCESS;
}
...
int CondInit (Cond *cond) {
	...
	if (AQueueInit (&cond->waiting) != QUEUE_SUCCESS) {
		printf("FATAL ERROR: could not initialize condition variable waiting queue in SemInit!\n");
		exitsim();
	}
	return SYNC_SUCCESS;
}
...
int CondWait (Cond *cond) {
	...
	if (locks[cond->lock].inuse != 1) {
		RestoreIntrs (intrval);
		return 1;
	}

	dbprintf ('I', "CondWait: Old interrupt value was 0x%x.\n", intrval);
	dbprintf ('c', "CondWait: Proc %d waiting on Cond %d.\n", GetCurrentPid(), (int)(cond-conds));
	dbprintf('c', "CondWait: putting process %d to sleep\n", GetCurrentPid());
	...
	if (AQueueInsertLast (&cond->waiting, l) != QUEUE_SUCCESS) {
		printf("FATAL ERROR: could not insert new link into Cond Var waiting queue in CondWait!\n");
		exitsim();
	}
	ProcessSleep();
	RestoreIntrs (intrval);
...
int CondSignal(Cond* cond) {
	...
	dbprintf ('c', "CondSignal: Process %d Signalling on Cond Var %d.\n", GetCurrentPid(), (int)(cond-conds));
	if (!AQueueEmpty(&cond->waiting)) { // there is a process to wake up
		...
		if ((l = AQueueAllocLink ((void *)pcb)) == NULL) {
