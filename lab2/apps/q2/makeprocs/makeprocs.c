...
void main (int argc, char *argv[])
{
	...
	for(i=0; i<numprocs; i++) {
		process_create(FILENAME_TO_RUN_P, h_mem_str, s_procs_completed_str, s_lock_str, h_buf, NULL);
		//Printf("Producer Process No. %d created\n", i);
	}
	for(i=0; i<numprocs; i++) {
		process_create(FILENAME_TO_RUN_C, h_mem_str, s_procs_completed_str, s_lock_str, h_buf, NULL);
