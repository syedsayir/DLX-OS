...
void main (int argc, char *argv[])
{
	...
	for(i=0;i<stringLen(h_buf);i++)
	{
		if (sem_wait(s_emptySlots) != SYNC_SUCCESS) {
			Printf("Bad semaphore s_emptySlots (%d) in ", s_emptySlots); Printf(argv[0]); Printf("\n");
			Exit();
		}
		...
		if(sem_signal(s_filledSlots) != SYNC_SUCCESS) {
			Printf("Bad semaphore s_filledSlots (%d) in ", s_filledSlots); Printf(argv[0]); Printf(", exiting...\n");
			Exit();
		}
