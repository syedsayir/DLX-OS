...
void main (int argc, char *argv[])
{
	...
	char c_isnotempty_str[10];          // Used as command-line argument to pass empty slots in buffer to new processes
	char c_isnotfull_str[10];          // Used as command-line argument to pass filled stots in buffer to new processes
	...
	if ((c_isnotfull = cond_create(s_lock)) == SYNC_FAIL) {
		Printf("Bad sem_create in "); Printf(argv[0]); Printf("\n");
		Exit();
	}
	if ((c_isnotempty = cond_create(s_lock)) == SYNC_FAIL) {
		Printf("Bad sem_create in "); Printf(argv[0]); Printf("\n");
		Exit();
	}
	...
	ditoa(c_isnotempty, c_isnotempty_str);
	ditoa(c_isnotfull, c_isnotfull_str);

	// Now we can create the processes.  Note that you MUST end your call to
	// process_create with a NULL argument so that the operating system
	// knows how many arguments you are sending.
	for(i=0; i<numprocs; i++) {
		process_create(FILENAME_TO_RUN_P, h_mem_str, s_procs_completed_str, s_lock_str, h_buf, c_isnotempty_str, c_isnotfull_str, NULL);
		//Printf("Producer Process No. %d created\n", i);
	}
	for(i=0; i<numprocs; i++) {
		process_create(FILENAME_TO_RUN_C, h_mem_str, s_procs_completed_str, s_lock_str, h_buf, c_isnotempty_str, c_isnotfull_str, NULL);
