...
int DfsOpenFileSystem() {
	...
	for (i=0; i<bound; i++) {
		DiskReadBlock(sb.FSinode_beg*2+i,(disk_block *)((char *)inodes+i*DISK_BLOCKSIZE));	
	}
	// Read free block vector
	bound=(sb.FSnumblocks*4)/(32*DiskBytesPerBlock());
	for (i=0; i<bound; i++) {
		DiskReadBlock(sb.FSblkvec_beg*2+i,(disk_block *)((char *)fbv+i*DISK_BLOCKSIZE));	
	}
...
int DfsCloseFileSystem() {
	...
	for (i=0; i<bound; i++) {
		DiskWriteBlock	(sb.FSinode_beg*2+i,(disk_block *)((char *)inodes+i*DISK_BLOCKSIZE));	
	}
	// Read free block vector
	bound=(sb.FSnumblocks*4)/(32*DiskBytesPerBlock());
	for (i=0; i<bound; i++) {
		DiskWriteBlock(sb.FSblkvec_beg*2+i,(disk_block *)((char *)fbv+i*DISK_BLOCKSIZE));	
	}
	// Change superblock to be invalid, write back to disk, then change 
	// it back to be valid in memory
...
int DfsReadBlock(uint32 blocknum, dfs_block *b) {
	...
	for(i=0;i<sb.FSblocksize/DISK_BLOCKSIZE;i++)
		DiskReadBlock(blocknum*2+i,(disk_block *)((char*)b+i*DISK_BLOCKSIZE));
...
int DfsWriteBlock(uint32 blocknum, dfs_block *b){
	...
	if(fbv[blocknum/32]&(1<<(blocknum%32))) return DFS_FAIL;
	for(i=0;i<sb.FSblocksize/DISK_BLOCKSIZE;i++)
		DiskWriteBlock(blocknum*2+i,(disk_block *)((char*)b+i*DISK_BLOCKSIZE));
	return sb.FSblocksize;
}
...
int DfsInodeDelete(uint32 handle) {
	...
	//check this -- need to lock before modifying
...
int DfsInodeAllocateVirtualBlock(uint32 handle, uint32 virtual_blocknum) {
	...
	if(!(inodes[handle].inUse)) { printf("1111\n\n"); return DFS_FAIL;}
	if (virtual_blocknum != inodes[handle].num_blocks){ printf("vbn:%d      nb:%d\n\n",virtual_blocknum , inodes[handle].num_blocks); return DFS_FAIL;} //allocating block = numblocks
