...
int ProcessRealFork() {
	...
	if ((pageNum = MemoryAllocPage()) == MEM_FAIL) {
		printf("Fail to allocate memory. Out of pages.\n");
		//process kill here.
		ProcessFreeResources (pcb);
		return (-1);
	}
	...
	pcb->currentSavedFrame = ((uint32) (currentPCB->currentSavedFrame) - (uint32) ((currentPCB->sysStackArea)) + (uint32) (pageNum<<MEM_L1FIELD_FIRST_BITNUM));

	pcb->sysStackPtr = ((uint32) (currentPCB->sysStackPtr) - (uint32)((currentPCB->sysStackArea)) + (uint32) (pageNum<<MEM_L1FIELD_FIRST_BITNUM));
	...
	for (i=0; i<MEM_L1TABLE_SIZE; i++) {
		if (currentPCB->pagetable[i]&MEM_PTE_VALID) {
			printf("Parent pagetable[%d]: 0x%x\n", i, currentPCB->pagetable[i]);
		}
	}
	printf("\nChild Page table\n");
	for (i=0; i<MEM_L1TABLE_SIZE; i++) {
		if (pcb->pagetable[i]&MEM_PTE_VALID) {
			printf("Child pagetable[%d]: 0x%x\n", i, pcb->pagetable[i]);
		}
	}
	printf("---------------------FROM FORK--------------------------------\n");
	//printf("returning %d\n",pcb-pcbs);
...
int ProcessFork (VoidFunc func, uint32 param, char *name, int isUser) {
	...
	pcb->sysStackArea = pageNum<<12;
	stackframe = (uint32*)(((pageNum<<MEM_L1FIELD_FIRST_BITNUM)|MEM_ADDRESS_OFFSET_MASK) & (~(0x03)));
	...
	pcb->pagetable[MEM_L1TABLE_SIZE-1] = (uint32)((pageNum<<MEM_L1FIELD_FIRST_BITNUM)|MEM_PTE_VALID);
	...
	for (i=0; i<4; i++) {
		...
		pcb->pagetable[i] =(uint32 )((pageNum<<MEM_L1FIELD_FIRST_BITNUM)|MEM_PTE_VALID);
