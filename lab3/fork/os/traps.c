	...
	if (cause & TRAP_TRAP_INSTR) {
		...
		switch (cause) {
			...
			case TRAP_PROCESS_FORK:
				...
				ProcessSetResult(GetAddressFromPid(ihandle), 0); //Return handle
				break;
			...
			case TRAP_COND_BROADCAST:
				...
				break;
			default:
				printf ("Got a unrecognized trap (0x%x) - exiting!\n",
						cause);
				exitsim ();
				break;
		}
		...
		//	printf("done ROP\n");
				break;
			default:
				printf ("Got %x b unrecognized system interrupt (0x) - exiting!\n",
						cause);
				exitsim ();
				break;
		}
