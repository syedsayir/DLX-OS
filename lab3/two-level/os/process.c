...
void ProcessModuleInit () {
	...
	for (i = 0; i < PROCESS_MAX_PROCS; i++) {
		...
		for (j=0; j<MEM_L1TABLE_SIZE; j++) {
			pcbs[i].pagetable[j] =0;// getL2PTAddress((i*MEM_L1TABLE_SIZE)+j);
			//printf("INIT:%x\n",pcbs[i].pagetable[j]);
		}
...
void ProcessFreeResources (PCB *pcb) {
	...
	pstack= pcb->sysStackArea;
	pstack=pstack>>MEM_L2FIELD_FIRST_BITNUM;
	//------------------------------------------------------------
	...
	for (i=0; i<MEM_L1TABLE_SIZE ;i++) {
		if (pcb->pagetable[i]) {
			for (j=0; j<MEM_L1TABLE_SIZE ;j++) {
				MemoryFreePage(i*MEM_L1TABLE_SIZE+j, pcb);
			}
		}
	}
	// Free the second level page table
	for (i=0; i<MEM_L1TABLE_SIZE ;i++) {
		if (pcb->pagetable[i]) {
			inUseAddr = (uint32 *) (pcb->pagetable[i] - 4);
			*inUseAddr = 0;
		}
	}
	FreePPage(pstack>>MEM_L2FIELD_FIRST_BITNUM);
...
int ProcessRealFork() {
	...
	printf("Entered PROCESSREALFORK\n");
	intrs = DisableIntrs ();
	...
	for (i=0; i<MEM_L1TABLE_SIZE; i++) {
		if (currentPCB->pagetable[i] & MEM_PTE_VALID) {
			pcb->pagetable[i] = currentPCB->pagetable[i];
			intrs = DisableIntrs ();
			dbprintf ('I', "Old interrupt value was 0x%x.\n", intrs);
			incCountRef(pcb->pagetable[i]>>MEM_L1FIELD_FIRST_BITNUM);
			RestoreIntrs (intrs);
		}
	}

	for (i=0; i<80; i++) {
		pcb->name[i] = currentPCB->name[i];
	}
	pcb->flags = currentPCB->flags;

	// System stack
	if ((pageNum = MemoryAllocPage()) == MEM_FAIL) {
		printf("1 Fail to allocate memory. Out of pages.\n");
		//process kill here.
		ProcessFreeResources (pcb);
		return (-1);
	}
	pcb->sysStackArea = pageNum<<12;

	pcb->currentSavedFrame = (uint32*) ((uint32) (currentPCB->currentSavedFrame) - (uint32) ((currentPCB->sysStackArea)) + (uint32) (pageNum<<MEM_L1FIELD_FIRST_BITNUM));

	pcb->sysStackPtr = (uint32*) ((uint32) (currentPCB->sysStackPtr) - (uint32)((currentPCB->sysStackArea)) + (uint32) (pageNum<<MEM_L1FIELD_FIRST_BITNUM));

	bcopy( (char*) ((currentPCB->sysStackArea)), (char*) ((pcb->sysStackArea)), MEM_PAGESIZE);
	...
	//printf("child System Stack------ %x\n", (pcb->sysStackArea));
	//printf("child System sysStackPtr------ 0x%x\n", (int) (pcb->sysStackPtr));
	//printf("parent System Stack------ %x\n", (currentPCB->sysStackArea));
	//printf("parent System sysStackPtr------ 0x%x\n", (int) (currentPCB->sysStackPtr));
	//printf("-------------------\n");
	//printf("parent offset------ 0x%x\n", ((int) ((currentPCB->sysStackArea)<<12)) - ((int) (currentPCB->sysStackPtr)));
	//printf("child offset------ 0x%x\n", ((int) ((pcb->sysStackArea)<<12)) - ((int) (pcb->sysStackPtr)));
	RestoreIntrs (intrs);
	printf("returning %d\n",(int) (pcb-pcbs));
	return pcb - pcbs;
...
int ProcessFork (VoidFunc func, uint32 param, char *name, int isUser) {
	...
	// System stack not in virtual Space
	if ((pageNum = MemoryAllocPage()) == MEM_FAIL) {
		printf("2 Fail to allocate memory. Out of pages.\n");
		//process kill here.
		ProcessFreeResources (pcb);
		return (-1);
	}
	pcb->sysStackArea = pageNum<<12;
	stackframe = (uint32*)(((pageNum<<MEM_L2FIELD_FIRST_BITNUM)|MEM_ADDRESS_OFFSET_MASK) & (~(0x03)));

	// User stack allocated to the last Valid Page of Virtual Space
	if ((pageNum = MemoryAllocPage()) == MEM_FAIL) {
		printf("3 Fail to allocate memory. Out of pages.\n");
		//process kill here. ------------------- todo
		ProcessFreeResources (pcb);
		return (-1);
	}
	...
	pcb->pagetable[MEM_L1TABLE_SIZE-1] = l2pageTable;
	((uint32*) pcb->pagetable[MEM_L1TABLE_SIZE-1])[MEM_L2TABLE_SIZE-1] = (uint32)((pageNum<<MEM_L2FIELD_FIRST_BITNUM)|MEM_PTE_VALID);
	//printf ("Sys Stack------%x\n\n",((uint32*) pcb->pagetable[MEM_L1TABLE_SIZE-1])[MEM_L2TABLE_SIZE-1]);
	...
	pcb->pagetable[0] = l2pageTable;
	for (i=0; i<4; i++) {
		if ((pageNum = MemoryAllocPage()) == MEM_FAIL) {
			printf("4 Fail to allocate memory. Out of pages.\n");
			//process kill here.
			ProcessFreeResources (pcb);
			return (-1);
		}
		((uint32*) pcb->pagetable[0])[i] =(uint32 )((pageNum<<MEM_L2FIELD_FIRST_BITNUM)|MEM_PTE_VALID|MEM_PTE_READONLY);

		//printf ("Page %d ------%x\n\n", i,(((uint32*) pcb->pagetable[0])[i]));
	}


	dbprintf('m', "Initial ProcessFork: stackframe = 0x%x\n", (int)stackframe);
