...
void ProcessFreeResources (PCB *pcb) {
	...
	// free the system stack page
	MemoryFreeStackPage(pcb);
...
int ProcessFork (VoidFunc func, uint32 param, char *name, int isUser) {
	...
	pcb->pagetable[MEM_L1TABLE_SIZE-1] = ((pageNum<<MEM_L1FIELD_FIRST_BITNUM)|MEM_PTE_VALID);
	...
	for (i=0; i<4; i++) {
		...
		pcb->pagetable[i] = ((pageNum<<MEM_L1FIELD_FIRST_BITNUM)|MEM_PTE_VALID);
